        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-lg sm:rounded-lg border border-gray-200 dark:border-gray-700">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <h3 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">Edit Skill</h3>
                    ...
                    <form action="{{ route('admin.skills.update', $skill) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div style="margin-bottom: 24px;">
                            ...
                            <input type="text" name="name" id="name" placeholder="Enter skill name" value="{{ $skill->name }}" required style="width: 100%; padding: 10px 12px; border: 2px solid #333; border-radius: 8px; font-size: 16px; font-family: 'Outfit', sans-serif;">
                            @error('name')
                        ...
                        <div style="margin-bottom: 24px;">
                            ...
                            <textarea name="description" id="description" rows="4" required style="width: 100%; padding: 10px 12px; border: 2px solid #333; border-radius: 8px; font-size: 16px; font-family: 'Outfit', sans-serif;">{{ $skill->description }}</textarea>
                            @error('description')
                                <p style="color: #D02020; font-size: 13px; margin-top: 4px;">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div style="margin-bottom: 24px;">
                            ...
                            <div style="border: 2px dashed #333; border-radius: 8px; padding: 24px; text-align: center; cursor: pointer;" id="upload-area">
                                ...
                                <div id="upload-prompt" style="display: {{ $skill->icon ? 'none' : 'block' }};">
                                ...
                                @if($skill->icon)
                                    <img id="preview" src="{{ asset('storage/' . $skill->icon) }}" style="max-width: 200px; max-height: 200px; margin-top: 12px; border-radius: 8px;">
                                @else
                        ...
                        <div style="display: flex; gap: 12px;">
                            ...
                            <a href="{{ route('admin.skills.index') }}" style="flex: 1; display: flex; align-items: center; justify-content: center; padding: 12px 16px; background-color: #f0f0f0; color: #333; border: 2px solid #121212; border-radius: 8px; font-weight: 600; font-size: 14px; text-decoration: none; transition: all 200ms ease-out;">
                                Cancel
                            </a>
                        </div>
                    </form>
